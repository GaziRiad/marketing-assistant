import React from "react";
import { HiInboxArrowDown, HiPencil } from "react-icons/hi2";
import { menuItemsContent } from "@/app/lib/constant";
import Image from "next/image";

const SideBar = () => {
  return (
    <div className="w-full rounded-[40px] bg-white pb-10 pt-10 md:h-[800px] md:w-1/2 xl:w-[262px]">
      <button className="footer-card-shadow ml-4 flex w-[131.3px] items-center justify-center gap-3 rounded-xl bg-[#C2E7FF] py-3 transition-all ">
        <HiPencil />
        <p className="text-[#545454]">Compose</p>
      </button>
      <div className="mt-5 flex h-[40px] w-[90%] cursor-pointer items-center justify-between rounded-r-3xl bg-[#F7F7F7] px-8">
        <div className="flex items-center gap-2">
          <HiInboxArrowDown size={18} />
          <p>Inbox</p>
        </div>
        <Image
          src={"/images/InBox.png"}
          alt="Logo"
          width={50}
          height={48}
          className=" transition-all hover:rotate-12"
        />
      </div>
      <div className="ml-8 mt-[10px] flex flex-col gap-y-3">
        {menuItemsContent.map((item, index) => (
          <div key={index} className="flex cursor-pointer items-center gap-2">
            {item.icon}
            <p>{item.label}</p>
          </div>
        ))}
      </div>
      <div className="mx-4 mt-10 grid grid-cols-2 gap-[9px]">
        <div className="hidden h-[228px] w-full rounded-2xl bg-[#F7F7F7] md:block" />
        <div className="hidden h-[228px] w-full rounded-2xl bg-[#F7F7F7] md:block" />
        <div className="hidden h-[130px] w-full rounded-2xl bg-[#F7F7F7] md:block" />
        <div className="hidden h-[130px] w-full rounded-2xl bg-[#F7F7F7] md:block" />
      </div>
    </div>
  );
};

export default SideBar;
